...
class getOffensiveActions(Actions):
    ...
    def getWeights(self, gameState, action):
            ...
            for agent in visible:
                if agent.scaredTimer > 0:
                    if agent.scaredTimer > 10:
                        return {'successorScore': 50, 'distanceToFood': -20, 'distanceToEnemiesPacMan': 0,
                                'GhostDistance': 0, 'distanceToCapsule': 0, 'returned': 0, 'carrying': 20}
                    # elif 3 < agent.scaredTimer <= 6 and numOfCarrying >= 7:
                    #     return {'successorScore': 510, 'distanceToFood': -3, 'distanceToEnemiesPacMan': 0,
                    #             'GhostDistance': 0, 'distanceToCapsule': -10, 'returned': -1000,
                    #             'carrying': 20}
                    elif 4 < agent.scaredTimer < 10 :
                        return {'successorScore': 110, 'distanceToFood': -10, 'distanceToEnemiesPacMan': 0,
                                'GhostDistance': -5, 'distanceToCapsule': -10, 'returned': -10,
                                'carrying': 20}

                # Visible and not scared
                else:
                    return {'successorScore': 110, 'distanceToFood': -10, 'distanceToEnemiesPacMan': 0,
                            'GhostDistance': 20, 'distanceToCapsule': -15, 'returned': -10,
                            'carrying': 0}
        ...
        if len(Range) > 0 and not successor.getAgentState(self.index).isPacman:
            return {'successorScore': 0, 'distanceToFood': -3, 'distanceToEnemiesPacMan': -500,
                    'distanceToCapsule': 0, 'GhostDistance': 0,
                    'returned': 0, 'carrying': 0}

        # Did not see anything
        return {'successorScore': 30, 'distanceToFood': -8, 'GhostDistance': 0, 'distanceToEnemiesPacMan': 0,
                'distanceToCapsule': -6, 'returned': 0, 'carrying': 35}
...
class getDefensiveActions(Actions):
    ...
    def PatrolCapsuleDistribution(self, gameState):
        """
        This method calculates the minimum distance from our patrol
        points to our pacdots. The inverse of this distance will
        be used as the probability to select the patrol point as
        ...
        # patrol points.
        for position in self.noWall:
            closestCapsuleDistance = 99999
            capsuleList = self.agent.getCapsulesYouAreDefending(gameState)
            for c in capsuleList:
                dist = self.agent.getMazeDistance(position, c)
                if dist < closestCapsuleDistance:
                    closestCapsuleDistance = dist

            # We can't divide by 0!
            if closestCapsuleDistance == 0:
                closestCapsuleDistance = 1
            self.patrolCapDict[position] = 1.0 / float(closestCapsuleDistance)
            total += self.patrolCapDict[position]
        # Normalize the value used as probability.
        if total == 0:
            total = 1
        for x in self.patrolDict.keys():
            self.patrolCapDict[x] = float(self.patrolCapDict[x]) / float(total)
    ...
    def selectCapusleTarget(self):
        rand = random.random()
        sum = 0.0
        for x in self.patrolDict.keys():
            sum += self.patrolCapDict[x]
            if rand < sum:
                return x
    ...
    def chooseAction(self, gameState):
        ...
        print(self.agent.getCapsulesYouAreDefending(gameState),"Capsuel!!!")
        ...
        # If we have many pacdots, let's patrol the map central area.
            ...
            self.target = self.selectCapusleTarget()
