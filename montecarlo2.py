...
sys.path.append("teams/<montecarlo2>/")
...
class getOffensiveActions(Actions):
    ...
    def getWeights(self, gameState, action):
            ...
            for agent in visible:
                if agent.scaredTimer > 0:
                    ...
                    return {'successorScore': 23, 'distanceToFood': -3, 'distanceToEnemiesPacMan': 0,
                            'GhostDistance': 1, 'distanceToCapsule': -5, 'returned': 0, 'carrying': 20}
                else:
                    return {'successorScore': 510, 'distanceToFood': -1, 'distanceToEnemiesPacMan': 0,
                            'GhostDistance': 10, 'distanceToCapsule': -51, 'returned': -100, 'carring': 20}
        ...
        #             'GhostDistance': 20, 'distanceToCapsule': 0, 'returned': 0, 'carring': 0}
        if gameState.getAgentState(self.index).numCarrying >= 7:
            return {'successorScore': 500, 'distanceToFood': 10, 'GhostDistance': 20, 'distanceToEnemiesPacMan': 0,
                    'distanceToCapsule': -55, 'returned': -1000, 'carrying': 0}

        return {'successorScore': 30, 'distanceToFood': -5, 'GhostDistance': 0, 'distanceToEnemiesPacMan': 0,
    ...
    def chooseAction(self, gameState):
        ...
        for a in actions:
            ...
            value = 0
            # for i in range(1, 31):
            #     value += self.randomSimulation1(1, new_state, 0.8) / 31
            # fvalues.append(value)
            value = self.allSimulation(2, new_state, 0.8)
            fvalues.append(value)
...
class getDefensiveActions(Actions):
    ...
    def chooseAction(self, gameState):
        ...
        visible = filter(lambda x: not x.isPacman and x.getPosition() != None or
                                   x.isPacman and x.getPosition() != None, opponentsState)
        if len(visible)>0:
            positions = []
